...
namespace task_buoy{
  ...
  void update_values(vision_buoy::buoyConfig &config, int flag, int blue_buoy[][2], int green_buoy[][2], int red_buoy[][2])
  {

    if (flag == 0){
      config.red_min_param = blue_buoy[0][0];
      config.red_max_param = blue_buoy[0][1];
      config.green_min_param = blue_buoy[1][0];
      config.green_max_param = blue_buoy[1][1];
      config.blue_min_param = blue_buoy[2][0];
      config.blue_max_param = blue_buoy[2][1];
    }
    else if (flag == 1){
      config.red_min_param = green_buoy[0][0];
      config.red_max_param = green_buoy[0][1];
      config.green_min_param = green_buoy[1][0];
      config.green_max_param = green_buoy[1][1];
      config.blue_min_param = green_buoy[2][0];
      config.blue_max_param = green_buoy[2][1];
    }
    else if (flag == 2){
      config.red_min_param = red_buoy[0][0];
      config.red_max_param = red_buoy[0][1];
      config.green_min_param = red_buoy[1][0];
      config.green_max_param = red_buoy[1][1];
      config.blue_min_param = red_buoy[2][0];
      config.blue_max_param = red_buoy[2][1];
    }

  }

  void threshold_values_update(int BGR[][2], vision_buoy::buoyConfig &config)
  {
    BGR[0][0] = config.red_min_param;
    BGR[0][1] = config.red_max_param;
    BGR[1][0] = config.green_min_param;
    BGR[1][1] = config.green_max_param;
    BGR[2][0] = config.blue_min_param;
    BGR[2][1] = config.blue_max_param;
  }
